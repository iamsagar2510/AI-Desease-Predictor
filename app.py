...
df = pd.read_csv("health_data.csv")  # <-- Change this to your actual data file

# Fit your encoders on your original training data
gender_encoder = LabelEncoder().fit(df["Gender"])
smoking_encoder = LabelEncoder().fit(df["Smoking"])
family_history_encoder = LabelEncoder().fit(df["FamilyHistory"])
disease_encoder = LabelEncoder().fit(df["Disease"])

encoders = {
    "Gender": gender_encoder,
    "Smoking": smoking_encoder,
    "FamilyHistory": family_history_encoder,
    "Disease": disease_encoder
}

# (Optional) Save encoders for future use
